...
using StorageItemVM = ArkDuckBot.Models.StorageItemVM;
    ...
    private static readonly Dictionary<string, string> sMonIconByKeyRaw = new(StringComparer.OrdinalIgnoreCase)
{
    ...
    private static async Task<bool> TryUpdateItemDbAsync()
    {
        ...
        try
        {
            ...
            client.DefaultRequestHeaders.UserAgent.ParseAdd("ArkDuckBot/1.0");
...
private sealed record MarkerRef(System.Windows.Shapes.Ellipse Dot, double U_DIP, double V_DIP, double Radius);
    ...
    public void HandleArkDuckBotLink(string link)
    {
        try
        {
            string host = "";
            int port = 27020; // Standard ARK ServerAPI Port
            ...
            {
                // --- FALL B: Manueller Link (z.B. arkduckbot://1.2.3.4:27020) ---
                // Wir entfernen das Protokoll "arkduckbot://"
                var raw = link.Replace("arkduckbot://", "").TrimEnd('/');
        ...
        {
            AppendLog("ArkDuckBot-Link-Error: " + ex.Message);
            MessageBox.Show("Unable to read ArkDuckBot-Link: " + ex.Message + "\n\nFormat: arkduckbot://IP:PORT");
        }
    }
    ...
    private (string host, int port, ulong playerId, int playerToken, string? name) ParseArkLink(string link)
    {
        ...
        // arkduckbot://?ip=…&port=…&playerid=…&playertoken=…
        // arkduckbot://add?address=…&port=…&playerid=…&token=…
        var l = link.Trim();

        // in normales Schema wandeln, damit Uri es versteht
        if (l.StartsWith("arkduckbot://", StringComparison.OrdinalIgnoreCase))
            l = "http://" + l["arkduckbot://".Length..]; // dummy-scheme
    ...
    private void UpdatePairingGuideSnackbar()
    {
        ...
        string msg = isListening 
            ? "Please pair your server in-game with DuckBot" 
