...
namespace ArkDuckBot;

public partial class App : Application
{
    private static Mutex? _single;
    private const string SingleMutexName = "ArkDuckBot_SingleInstance";
    private const string PipeName = "ArkDuckBotLinkPipe";
    ...
    protected override void OnStartup(StartupEventArgs e)
    {
        ...
        {
            // Already running
            if (e.Args.Length > 0 && e.Args[0].StartsWith("arkduckbot://", StringComparison.OrdinalIgnoreCase))
        ...
        if (e.Args.Length > 0 && e.Args[0].StartsWith("arkduckbot://", StringComparison.OrdinalIgnoreCase))
            _main?.HandleArkDuckBotLink(e.Args[0]);
    }
    ...
    private void SetupTrayIcon()
    {
        ...
        _trayIcon.Text = "ArkDuckBot Tracker";
        ...
        menu.Opening += (s, e) =>
        {
            ...
            menu.Items.Add("Open ArkDuckBot", null, (s, ex) => ShowMainWindow());
            menu.Items.Add(new System.Windows.Forms.ToolStripSeparator());
        ...
        TrackingService.OnOnlinePlayersUpdated += () => {
            ...
            Dispatcher.Invoke(() => {
                try {
                    if (_trayIcon != null)
                        _trayIcon.Text = $"ArkDuckBot (Tracking {last})";
                } catch { }
            });
        };
    }
    ...
    private static void EnsureUrlProtocolRegistered()
    {
        try
        {
            const string scheme = "arkduckbot";
            using var key = Registry.CurrentUser.CreateSubKey($@"Software\Classes\{scheme}");
            key.SetValue("", "URL: arkduckbot Protocol");
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(link) && _main != null)
                {
                    _main.Dispatcher.Invoke(() =>
                    {
                        ...
                        else if (link.StartsWith("arkduckbot://", StringComparison.OrdinalIgnoreCase))
                        {
                            ShowMainWindow();
                            _main.HandleArkDuckBotLink(link);
                        }
                    });
                }
                else if (link == "SHOWUI")
